...
let createMovie = async (
  ...
  const addedMovie = await getMovie(insertMovie.insertedId.toString());
  return addedMovie;
...
let getSort = async (value) => {
  if (!value) throw "missing input parameter";
...
let searchByMovie = async (movie) => {
  ...
  const moviesCollection = await movies();
  let matched = await moviesCollection
    .find({ movie_name: regExTerm })
    .toArray();
...
let searchByDirector = async (director) => {
  ...
  if (typeof director !== "string") throw "invalid data type";

  if (director.trim().length === 0) throw "invalid strings";
...
let searchByCast = async (name) => {
  ...
  if (name.trim().length === 0) throw "invalid strings";

  let regExTerm = new RegExp(".*" + name.trim() + ".*", "i");
  const moviesCollection = await movies();
  let matched = await moviesCollection
    .find({ cast: { $in: [regExTerm] } })
    .toArray();
  if (matched.length === 0) throw `no movies matched to ${director.trim()}.`;
...
const updateMovieReport = async (movieId, username) => {
  // ------------------- ERROR CHANGES ----------------------
  if (!movieId || !username) throw "no movieId was supplied";
...
let deleteMovie = async (id) => {
  if (!id) throw "no id was supplied";
